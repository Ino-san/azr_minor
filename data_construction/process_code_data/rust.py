...
def extract_input_from_rust_string(rust_code_content: str) -> str:
    # Extract the input from the Rust code content
    input_match = re.search(r'assert_eq\!\(candidate(.|\n)*?\);', rust_code_content)
    if input_match:
        input = input_match.group(0)
        input = re.sub(r'assert_eq\!\(candidate\(', '', input)
        input = re.sub(r'\), \?\?\?\?\);', '', input)
        return input
    else:
        raise ValueError("Could not find input in Rust code content")

def extract_output_from_rust_string(rust_code_content: str) -> str:
    output_match = re.search(r'candidate\(\?\?\?\?(.|\n)*?\);', rust_code_content)
    if output_match:
        output = output_match.group(0)
        output = re.sub(r'candidate\(\?\?\?\?\), ', '', output)
        output = re.sub(r'\)\;', '', output)
        return output
    else:
        raise ValueError("Could not find output in Rust code content")

def extract_function_from_rust_string(rust_code_content: str) -> str:
    extracted_lines = []
    in_function = False
    brace_level = 0
    ...
    # 文字列を改行で分割して行リストにする
    lines = rust_code_content.splitlines(keepends=True) # keepends=Trueで改行文字を保持
    for line in lines:
...
if __name__ == '__main__':
    ...
    # 283, 452, 510
    ds = load_dataset('xhwl/cruxeval-x')['Rust']
    ds = ds.map(lambda x: {'problem': extract_function_from_rust_string(x['code']),
                            'input': extract_input_from_rust_string(x['output_reasoning']),
                            'output': extract_output_from_rust_string(x['input_reasoning'])
                           })
    output_data = []
    print(ds[0]['input'])
    ...
    df.to_parquet(path / f'rust_test_answer{"_" + str(args.max_length) if args.max_length > 0 else ""}.parquet')
